...
public class Comp3 implements DataType {
	...
	private static BigInteger[] min = new BigInteger[39];
	private static BigInteger[] max = new BigInteger[39];
	...
	private void validateSize() {
		if (!this.signed && this.biValue.compareTo(BigInteger.valueOf(0L)) == -1) {// (< 0)
			throw new ArithmeticException(CAN_NOT_FIT + this.biValue + " into pic 9(" + this.cobolPadSize + COMP_3);
		} else if (this.signed && this.biValue.compareTo(min[this.cobolPadSize]) == -1) {
			throw new ArithmeticException(CAN_NOT_FIT + this.biValue + " into pic s9(" + this.cobolPadSize + COMP_3);
		} else if (this.biValue.compareTo(max[this.cobolPadSize]) == 1) {
			if (this.signed) {
