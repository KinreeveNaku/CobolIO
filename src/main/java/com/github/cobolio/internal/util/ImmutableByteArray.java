 ...
 */
public class ImmutableByteArray implements Comparable<ImmutableByteArray> {
	...
	public static ImmutableByteArray wrap(byte[] bytes) {
		if (bytes == null)
			return null;
	...
	/**
	 * Create a ImmutableByteArray using the byte array.
	 *
	...
	/**
	 * Get the data from the ImmutableByteArray.
	 * 
	...
	public boolean equals(Object other) {
		...
		if (other instanceof ImmutableByteArray)
			return Arrays.equals(this.bytes, ((ImmutableByteArray) other).get());

		return false;
	}

	@Override
	public int compareTo(ImmutableByteArray that) {
		return BYTES_LEXICO_COMPARATOR.compare(this.bytes, that.bytes);
	}

	@Override
	public String toString() {
		return ImmutableByteArray.toString(bytes, 0, bytes.length);
	}
	 ...
	 * IndexOutOfBoundsException if incrementing would cause the underlying input
	 * byte array to overflow.
	 *
	 * @param input - The byte array to increment
	 * @return A new copy of the incremented byte array.
	 */
	public static ImmutableByteArray increment(ImmutableByteArray input) throws IndexOutOfBoundsException {
	...
	private static class LexicographicByteArrayComparator implements ByteArrayComparator {
		...
		public int compare(final byte[] buffer1, int offset1, int length1, final byte[] buffer2, int offset2,
				int length2) {
